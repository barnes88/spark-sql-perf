...
val baseLocation = s"/scratch/tgrogers-disk01/a/barnes88/spark-sql-perf" // S3 bucket, blob, or local root path
val baseDatagenFolder = "/scratch/tgrogers-disk01/a/barnes88/spark-sql-perf/dbricksgen"  // usually /tmp if enough space is available for datagen files
...
val intScaleFactors = Seq(1) //set scale factors to run
val format = "parquet" //format has have already been generated

def perfDatasetsLocation(scaleFactor: Int, format: String) = 
  s"/scratch/tgrogers-disk01/a/barnes88/spark-sql-perf/tpch/tpch_sf${scaleFactor}_${format}"

val resultLocation = "/scratch/tgrogers-disk01/a/barnes88/spark-sql-perf/results"
val iterations = 2
...
intScaleFactors.foreach{ scaleFactor =>
